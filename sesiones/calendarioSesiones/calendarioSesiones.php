...
/**
 * @titulo Calendario con sesiones
 * @descripcion Ejemplo de un calendario básico en php con sesiones
...
?>
    ...
    <head>
        ...
        <title>Calendario hecho en Clase Sesiones</title>
    ...
    <body>
        ...
        if ($ldisplayCalendario) {
            if(isset($_GET["fecha"])){
                ...
                $diaActual = $datosFecha[0];
                $mesActual = $datosFecha[1];
                $anyoActual = $datosFecha[2];
            }

            $fecha = "$diaActual"+"$diaActual"+"$anyoActual";
            echo "Fecha:";
            echo "<input type='text' name='tarea' value='" . $tarea . "'>";
            ...
            for ($i = 1; $i <= 35; $i++) {
                ...
                if ($contadorColumnas % 7 == 0)
                    echo "<td style='background-color:red;'><a href='".htmlspecialchars($_SERVER['PHP_SELF'])."?fecha=$strfecha' method='post'>$i</td>";
                else if ($contadorColumnas % 7 == 0)
                    echo "<td style='background-color:red'>$i</td>";
                else
                    echo "<td><a href='".htmlspecialchars($_SERVER['PHP_SELF'])."?fecha=$fecha' method='post'>$i</td>";
                if ($contadorColumnas % 7 == 0)
                    echo "</tr><tr>";
                if ($i >= $diasMes)
                    return;
